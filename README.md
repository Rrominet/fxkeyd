...
`fxkeyd` is a fork of the original software [Keyd](https://github.com/rvaiya/keyd) wich is a daemon which remaps keys using kernel level input primitives (evdev, uinput).
It's really usefull to create new layer or new possible combinaison of keys on your keyboard at the software level.

## Why this fork for fxos ?

I needed to be able to remove a keyboard device from the list of available input devices.  
It's really easy on `X11` using `xinput disable` but when you use `keyd`, it create a virtual keyboard device and so `xinput` don't works because it's listening on the virtual keyboard device.  
And so the only thing I found acceptable was to directly ask to `keyd` to ignore my device.  
Sadly, I didn't find a way to do this directly in `keyd` so I created this fork.
...
Create the file `/etc/keyd/devices_ignored` with the devices name you want to ignore (one per line).
You can use `xinput list` to get the name of the devices.

Restart the `fxkeyd` service with `sudo systemctl restart fxkeyd`
And you're done.

________

FYI, below here is the `README` of the original repo [keyd](https://github.com/rvaiya/keyd) :
